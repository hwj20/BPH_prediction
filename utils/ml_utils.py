...
def train_all(train_features, train_labels, test_features, test_labels):
    ...
    if draw_comparison:
        ...
        sns.set_style("whitegrid")
        colors = ['#5cb85c', '#5bc0de', '#d9534f', '#9b59b6', '#34495e']  # �Զ�����ɫ
        ...
        ax[4][0].tick_params(labelsize=12)  # ����x���ǩ�����С
        # ax.set_ylim([0.5, 1.0])
        # ax.set_yticks(np.arange(0.5, 1.01, 0.25))  # set y-axis ticks from 0.5 to 1.0 with step 0.25
        ...
        yticks = [0.5, 0.75, 1.0]
        for i in range(5):
            ax[i][0].set_ylim([0.5, 1])  # ����ÿ����ͼ��y�᷶Χ
            ax[i][0].set_yticks(yticks)  # ����y��̶ȳ�
            ax[i][0].set_yticklabels(yticks, fontsize=12)  # ����y��̶ȳ߱�ǩ�������С
            ax[i][0].legend(loc=(1.01, 1))

        plt.tight_layout()  # �ս�ͼ�񲼾�
